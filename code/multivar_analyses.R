...
split_nmds_k2_vulner = split(nmds_k2_vis_data_vulner, nmds_k2_vis_data_vulner$vulner_score)
...
split_nmds_k2_diet = split(nmds_k2_vis_data_diet, nmds_k2_vis_data_diet$diet_study)
...
combined_nmds_k2_diet = combined_nmds_k2_diet %>% 
  mutate(diet_cat = ifelse(combined_nmds_k2_diet$diet_study == 1, 'Present', 'Absent'))

nmds_k2_plot_diet <- ggplot(data=combined_nmds_k2_diet) + 
  geom_polygon(aes(x = NMDS1, y = NMDS2, fill= fct_relevel(diet_cat, 'Absent', 'Present'), 
                   group=fct_relevel(diet_cat, 'Absent', 'Present')), alpha=0.30) + # add the convex hulls
  ...
  theme(axis.text.x = element_blank(),  # remove x-axis text
        ...
        panel.grid.minor = element_blank(),  #remove minor-grid labels
        plot.background = element_blank()) +
  scale_fill_manual('Presence in Diet Studies', values = c('#000080', '#ed2939')) +
  scale_colour_manual('Presence in Diet Studies', values = c('#000080', '#ed2939'))
