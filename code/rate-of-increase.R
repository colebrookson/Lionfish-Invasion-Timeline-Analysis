...
reef_abund_full$date = as.Date(with(reef_abund_full, 
...
#NOTE --- decided to exclude any subregions which were invaded after 2015 
# (will do this later in the script)
...
reef_abund_lf_minmax = reef_abund_lf %>% 
  ...
  filter(min_date < '2015-01-01') %>%  #isn't needed but keeping it 4 posterity
  filter(max_abund > 2)
...
for(i in unique(reef_abund_lf_minmax$subregion)) {
  ...
  abund = reef_abund_lf_minmax[which(reef_abund_lf_minmax$subregion == i), 
                               'max_abund'][[1]]
  ...
  reef_abund_lf_minmax[which(reef_abund_lf_minmax$subregion == i), 
                       'max_date'] = min(df$date)
...
for(i in unique(reef_abund_lf_minmax$subregion)) {
  ...
  year_mid = as.numeric(substr(as.character(
    reef_abund_lf_minmax[which(reef_abund_lf_minmax$subregion == i), 
                         'mid_date'][[1]]),
                start = 1, stop = 4))
  month_mid = as.numeric(substr(as.character(
    reef_abund_lf_minmax[which(reef_abund_lf_minmax$subregion == i), 
                         'mid_date'][[1]]),
                start = 6, stop = 7))
  ...
  if(nrow(df)>0) {
    ...
    reef_abund_lf_minmax[which(reef_abund_lf_minmax$subregion == i), 
                         'mid_abund'] = ceiling(mean(df$Abundance))
...
reef_abund_lf_minmax$time_max_abund = 
  ((reef_abund_lf_minmax$max_date - reef_abund_lf_minmax$min_date)/365)
reef_abund_lf_minmax$col = 
  ifelse(reef_abund_lf_minmax$time_max_abund > 8,
...
time_max_abund_plot = ggplot(data = reef_abund_lf_minmax) +
  ...
  labs(x = paste0('Time Elapsed Between First Invasion', 
...
rate_increase_min = reef_abund_lf_minmax %>% 
  ...
  filter(subregion %in% 
           unique(reef_abund_lf_minmax$subregion[which(
             reef_abund_lf_minmax$mid_abund > 0)]))
...
#stitch together min mid and max to plot the dates on the x abund on the y
...
write.table(reef_abund_lf_minmax, 
            here('./data/rate_of_increase_tble.txt'), sep = ',')
...
lf_abund_monthly = reef_abund_lf %>% 
  ...
  summarize(lf_mean = mean(Abundance),
            n = n(),
            lf_low = mean(Abundance) - 
              (qnorm(0.975)*(sd(Abundance)/sqrt(n))),
            lf_high = mean(Abundance) + 
              (qnorm(0.975)*(sd(Abundance)/sqrt(n)))) %>% 
  filter(n > 3)
lf_abund_monthly$lf_low = ifelse(lf_abund_monthly$lf_low > 0, 
                                 lf_abund_monthly$lf_low,
                                 0) 
...
ggplot(data = lf_abund_monthly) +
  ...
  geom_ribbon(aes(x = year, ymin = lf_low, ymax = lf_high, 
                  colour = subregion, fill = ribbon),
              size = 1.05, alpha = 0.4) +
  scale_fill_manual('', labels = c('',''), values = c('blue3', 'grey90')) +
  scale_colour_manual('Subregion', 
                      values = c('#ca3433', '#ffd300', '#ffd300', '#006994'),
                      labels = c('North Florida (Atlantic)', 'Florida Keys', 
                                 'Honduras', 'Leeward Islands')) +
  theme_bw() +
  theme(panel.grid = element_blank(),
        axis.title.y = element_text(size = 16),
        axis.title.x = element_text(size = 16),
...
ggsave(here('./figures/hist_and_timeseries_opt2_small.png'), 
       height = 6, width = 13,
       figure_7, dpi = 200)
ggsave(here('./figures/hist_and_timeseries_opt2_large.png'), 
       height = 6, width = 13,
       figure_7, dpi = 600)
